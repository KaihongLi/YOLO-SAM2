...
class BaseVideoDataLoader(DataLoader):
    def __init__(self, dataset, batch_size, num_workers):
        ...
        super(BaseVideoDataLoader, self).__init__(**self.init_kwargs)
...
def collate_fn(batch):
    ...
    for sample in batch:
        ...
        images_dir.append(sample[1])
        labels.append(sample[2])
        if sample[3] is not None and sample[4] is not None:
            categories.append(sample[3])
            bboxes.append(sample[4])
        images_id.append(sample[5])
        images_index.append(sample[6])
    return images, images_dir, labels, categories, bboxes, images_id, images_index
