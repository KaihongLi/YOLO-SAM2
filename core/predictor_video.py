...
Predict object masks and save visualisation results(Video)
"""
import logging
import os
...
class PredictorVideo(object):
    ...
    def inference(self, visual_dir: str):
        self.logger.info("YOLO-SAM2 Video Segmentation inference")
        ...
        for batch_idx, (data, data_dir, target, categories_id, bboxes, images_id, images_index) in enumerate(tbar):
            ...
            # Visual
            for image, boxes, masks, category_id, image_id in zip(data, new_bboxes, new_masks_batch, new_categories_id, images_id):
                plt.figure(figsize=(8, 6))
                plt.imshow(image)
                for mask in masks:
                    show_mask(mask.squeeze(0) if mask.ndim == 3 else mask, plt.gca(), random_color=True)
                for i in range(len(boxes)):
                    show_box(boxes[i], plt.gca(), category_id[i], None, self.classes, self.palette)
                plt.axis('off')
