"""
    ...
    ImageNet VID Video Dataset
"""
from pathlib import Path

from base import BaseVideoDataSet, BaseVideoDataLoader
import numpy as np
import os
...
class VIDVideoDataset(BaseVideoDataSet):
    ...
    def __init__(self, num_classes, conf, **kwargs):
        ...
        super(VIDVideoDataset, self).__init__(**kwargs)
    ...
    def _set_files(self):
        ...
        for video in videos:
            ...
            image_list = [os.path.join(video, image.split('.')[0]) for image in image_list]
    ...
    def _load_data(self, path):
        ...
        if self.prompt_file is not None:
            ...
            if len(bboxes) > 10:
                ...
                bboxes = bboxes[:10]
            return image, image_dir, label, categories_id, bboxes, image_id, image_index
        else:
            return image_path, image_dir, label, None, None, image_id, image_index


class VIDVideo(BaseVideoDataLoader):
    def __init__(self, config, prompt_file, conf):
        ...
        if split in ["train", "trainval", "val", "test"]:
            self.dataset = VIDVideoDataset(num_classes, conf, **kwargs)
        else:
            raise ValueError(f"Invalid split name {split}")
        super(VIDVideo, self).__init__(self.dataset, config["dataloader"]["batch_size"], config["dataloader"]["num_workers"])
