...
if __name__ == '__main__':
    parser = ArgumentParser(description='YOLO-SAM2 Evaluator')
    parser.add_argument(
        "-c",
        "--config",
    ...
    evaluator = Evaluator(yolo_model, sam_predictor, data_loader, args.conf_thres, device)

    evaluator.evaluate(config["predictor"]["save_dir"], config["auto_mask"]["save_dir"])

    # predictor = Predictor(yolo_model, sam_predictor, data_loader, args.conf_thres, device)
    #
    # predictor.inference(config["predictor"]["save_dir"])
